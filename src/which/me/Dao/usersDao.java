...
public class usersDao {
	...
	public static JSONArray selAll(int p,int pageSize){
		String sql = "select * from t_user limit " + pageSize;
		return sel(sql);
	}
	/**
	 * 根据昵称查询
	...
	public static JSONArray nameSel(int p,int pageSize,String username){
		String sql = "select * from t_user where userName like '%"+username+"%' limit " + pageSize;
		return sel(sql);
	}
	/**
	 * 查询sql 返回json集合
	...
	public static int add(userBean u){
		String sql = "insert into t_user(userName, trueName, sex, birthday, statusID, phone, address, email, userType, password) values(?,?,?,?,?,?,?,?,?,?)";
		Connection con = Conn.getCon();
		PreparedStatement ps = null;
		int i = 0;
		try {
