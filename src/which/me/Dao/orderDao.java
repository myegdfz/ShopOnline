...
public class orderDao {
	...
	public static PageBean userIdIsorder(int userId,int p,String type){
		...
		switch (Integer.parseInt(type)) {
			case 1:
				sql2 = "select * from t_order where userId=" +userId + " order by time desc limit "+pb.getP() +"," +pb.getPagesize();
				break;
			case 2:
				sql2 = "select * from t_order where userId ="+userId +" and state=1 order by time desc limit "+pb.getP() +"," +pb.getPagesize();
				break;
			case 3:
				sql2 = "select * from t_order where userId ="+userId + " and state = 6 order by time limit " +pb.getP() +"," +pb.getPagesize();
				break;
			case 4:
				sql2 = "select * from t_order where userId ="+userId + " and state = 3 order by time limit "+pb.getP() +"," +pb.getPagesize();
				break;
			default:
				break;
		}
	...
	public static JSONArray selAll(int p,int pageSize){
		String sql = "select t_order.*, t_user.userName, t_user.trueName, t_address.province, t_address.city, t_address.area, t_address.phone, t_address.username "
				+ "from t_order, t_user, t_address where t_order.userId=t_user.id and t_address.id=t_order.addressId order by time desc limit "+(p-1)*pageSize+"," + pageSize;
		return sel(sql);
	}
	/**
	 * 查询sql 返回json集合
	...
	public static int add(orderBean u){
		...
		String sql = "insert into t_order(id, userId, total, addressId, remarks, time, state) values(?,?,?,?,?,?,?)";
		Connection con = Conn.getCon();
	...
	public static int update(String orderId,String addressId,String PayType,String liuyan){
		...
		try {
			ps = con.prepareStatement(sql);
			System.out.println("lll : " + addressId);
