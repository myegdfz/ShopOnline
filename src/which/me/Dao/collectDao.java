...
public class collectDao {
	...
	public static int add(collectBean u){
		String sql = "insert into t_collect(userId, goodsId, goodsName, goodsProPic, goodsPrice, time) values(?,?,?,?,?,?)";
	...
	public static List selList(int userId,int page){
		...
		String sql = "select * from t_collect where userId =" +userId +" order by time desc limit " + (page-1)*10 + ",10";
		...
		try {
			...
			rs = ps.executeQuery();
			while(rs.next()){
				collectBean b = new collectBean(userId, rs.getInt("goodsId"), rs.getString("goodsName"),
						rs.getString("goodsProPic"), rs.getDouble("goodsPrice"), rs.getString("time"));
