...
public class goodsDao {
	...
	public static PageBean sidPageSel(String sid,int p,String order){
		...
		String sql2 = null;
		if(order.equals("1")){
			sql2 = "select id, name, price, proPic from t_goods where smallTypeId = "+sid + " order by views desc limit "+(pb.getP()-1)*pb.getPagesize()
					+","+ pb.getPagesize();
			
		}else if(order.equals("2")){
			sql2 = "select id, name, price, proPic from t_goods where smallTypeId ="+sid+" order by id desc limit " +(pb.getP()-1)*pb.getPagesize()
					+","+ pb.getPagesize();
		}else if(order.equals("3")){
			sql2 = "select id, name, price, proPic from t_goods where smallTypeId ="+sid+" order by price desc limit "+(pb.getP()-1)*pb.getPagesize()
					+","+ pb.getPagesize();
		}else if(order.equals("4")){
			sql2 = "select id, name, price, proPic from t_goods where smallTypeId ="+sid+" order by sales desc limit "+(pb.getP()-1)*pb.getPagesize()
					+","+ pb.getPagesize();
		}
	...
	public static PageBean bidPageSel(String bid,int p,String order){
		...
		String sql2 = null;
		if(order.equals("1")){
			sql2 = "select id,name, price, proPic from t_goods where bigTypeId = " +bid+" order by views desc limit " +(pb.getP()-1)*pb.getPagesize()
					+","+ pb.getPagesize();
		}else if(order.equals("2")){
			sql2 = "select id,name, price, proPic from t_goods where bigTypeId = " +bid+" order by id desc limit " +(pb.getP()-1)*pb.getPagesize()
					+","+ pb.getPagesize();
		}else if(order.equals("3")){
			sql2 = "select id,name, price, proPic from t_goods where bigTypeId = " +bid+" order by price desc limit " +(pb.getP()-1)*pb.getPagesize()
					+","+ pb.getPagesize();
		}else if(order.equals("4")){
			sql2 = "select id,name, price, proPic from t_goods where bigTypeId = " +bid+" order by sales desc limit "+(pb.getP()-1)*pb.getPagesize()
					+","+ pb.getPagesize();
		}
		System.out.println("分页查询语句为："+sql2);
		List<goodsBean> list = new ArrayList<goodsBean>(); //实例化一个商品集合
	...
	public static PageBean pageSel(String name,int p,String order){
		...
		System.out.println("获取到查询的条数为："+count);
		pb.setCount(count); //放入总条数
		System.out.println(p+" : " + pb.getPagesize() + " : or " +order);
		pb.setP(p); //放入当前页码
		System.out.println("pb.getP()="+pb.getP());
		String sql2 = null;
		if(order.equals("1")){
			sql2 = "select id, name, price, proPic from t_goods where name like '%"+name+"%' order by views desc limit "+(pb.getP()-1)*pb.getPagesize()
					+","+ pb.getPagesize();
		}else if(order.equals("2")){
			sql2 = "select id, name, price, proPic from t_goods where name like '%"+name+"%' order by id desc limit " +(pb.getP()-1)*pb.getPagesize()
					+","+ pb.getPagesize();
		}else if(order.equals("3")){
			sql2 = "select id, name, price, proPic from t_goods where name like '%"+name+"%' order by price desc limit " +(pb.getP()-1)*pb.getPagesize()
					+","+ pb.getPagesize();
		}else if(order.equals("4")){
			sql2 = "select id, name, price, proPic from t_goods where name like '%"+name+"%' order by views sales limit " +(pb.getP()-1)*pb.getPagesize()
					+","+ pb.getPagesize();
		}
		System.out.println("分页查询语句为："+sql2);
		List<goodsBean> list = new ArrayList<goodsBean>(); //实例化一个商品集合
	...
	public static List bigTypeIdSelxg(int bigTypeId){
		String sql = "select id, name, price, proPic from t_goods where bigTypeId ="+bigTypeId + " order by sales desc limit 6";
		Connection con = Conn.getCon();
		ResultSet rs = null;
		List<goodsBean> list = new ArrayList<goodsBean>();
		try {
	...
	public static JSONArray sel(String sql){
		...
		try {
			...
			while(rs.next()){
				...
				mapOfColValues.put("price", rs.getObject(3)); //获取列名和值放进JSON传输对象中
				mapOfColValues.put("proPic", rs.getObject(4)); //获取列名和值放进JSON传输对象中
				mapOfColValues.put("brand", rs.getObject(5)); //获取列名和值放进JSON传输对象中
				mapOfColValues.put("sales", rs.getObject(6)); //获取列名和值放进JSON传输对象中
				mapOfColValues.put("views", rs.getObject(7)); //获取列名和值放进JSON传输对象中
				mapOfColValues.put("stock", rs.getObject(8)); //获取列名和值放进JSON传输对象中
				mapOfColValues.put("contents", rs.getObject(9)); //获取列名和值放进JSON传输对象中
				mapOfColValues.put("bigTypeId", rs.getObject(10)); //获取列名和值放进JSON传输对象中
				mapOfColValues.put("smallTypeId", rs.getObject(11)); //获取列名和值放进JSON传输对象中
				mapOfColValues.put("state", rs.getObject(12)); //获取列名和值放进JSON传输对象中
				mapOfColValues.put("smallTypeName", rs.getObject(13)); //获取列名和值放进JSON传输对象中
				mapOfColValues.put("bigTypeName", rs.getObject(14)); //获取列名和值放进JSON传输对象中
	...
	public static JSONArray nameSel(int p,int pageSize,String name){
		...
		String sql = "select t_goods.*, t_smallType.name smallTypeName, t_bigtype.name bigTypeName from t_goods, t_smalltype, t_bigtype where t_smalltype.id = t_goods.smallTypeId"+
				" and t_goods.name like '%"+name +"%' and t_bigtype.id = t_goods.bigTypeId order by t_goods.id desc limit  "+(p-1)*pageSize+"," +pageSize;
		return sel(sql);
	}
	public static JSONArray selAll(int p,int pageSize){
		String sql = "select t_goods.*, t_smalltype.name smallTypeName, t_bigtype.name bigTypeName from t_goods, t_smalltype, t_bigtype "+
				"where t_smalltype.id = t_goods.smallTypeId and t_bigType.id = t_goods.bigTypeId order by t_goods.id desc limit "+(p-1)*pageSize+"," +pageSize;
		return sel(sql);
	}
	/**
	 * 查询总行数
	...
	public static int add(goodsBean u){
		String sql = "insert into t_goods(name, price, proPic, brand, sales, views, stock, contents, bigTypeId, smallTypeId, state) values(?,?,?,?,?,?,?,?,?,?,?)";
		Connection con = Conn.getCon();
		PreparedStatement ps = null;
		int i = 0;
		try {
