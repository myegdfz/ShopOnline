<%@page import="which.me.Bean.goodsBean"%>
<%@page import="which.me.Bean.bigTypeBean"%>
    ...
    <div class="nav-all"> 
     ...
     <div class="all-goods" style="display: none;" id="all-goods"> 
      ...
      <c:forEach items="${bigTypes }" var="bt">
         <div class="item"> 
          <div class="product">
           <h4><a href="searchServlet?bid=${bt.id }&p=1">${bt.name }</a> </h4>
          </div> 
           ...
           <div class="cf"> 
            <div class="fl wd252 pr20"> 
             ...
             <c:forEach items="${bt.smallTypeList }" var="sm">
              <li><span><a href="searchServlet?sid=${sm.id }&p=1">${sm.name }</a></span> </li> 
             </c:forEach>
             </ul>
            </div> 
           </div> 
     ...
     <div class="goodlist-main"> 
    		...
    		<c:forEach items="${bigTypes }" var="bt" >
    			...
    			<c:if test="${bt.id==param.bid }">
    				<c:forEach items="${bt.smallTypeList }" var="small">
         				<span><a href="searchServlet?sid=${small.id }&p=1">${small.name }</a></span>
    				</c:forEach>
    			</c:if>
    		</c:forEach>
        </div> 
      ...
      <div class="lists-area"> 
       ...
       <ul class="ml_content_top">
        <li <c:if test="${param.order=='1' || param.order==null }">class="active"</c:if> ><a href="searchServlet?${type }=${s }&p=1&order=1" class="asc">热度<i></i></a></li>
        <li <c:if test="${param.order=='2' }">class="active"</c:if> ><a href="searchServlet?${type }=${s }&p=1&order=2" class="asc">最新<i></i></a></li>
        <li <c:if test="${param.order=='3' }">class="active"</c:if> ><a href="searchServlet?${type }=${s }&p=1&order=3" class="asc">价格<i></i></a></li>
        <li <c:if test="${param.order=='4' }">class="active"</c:if> ><a href="searchServlet?${type }=${s }&p=1&order=4" class="asc">销量<i></i></a></li>
       </ul> 
       <ul class="goodlist"> 
       ...
       <div class="page"> 
        <div> 
        	<c:if test="${param.p!=1  }">
        		<a class="prev" href="searchServlet?${type }=${s }&p=${param.p-1 }&order=${order }#s">上一页</a>
        	</c:if>
        	<c:forEach begin="1" end="${PageDate.pagetotal }" varStatus="sta">
        		...
        		<c:if test="${param.p!=sta.count }">
        			<a class="num" href="searchServlet?${type }=${s }&p=${sta.count }&order=${order }#s">${sta.count }</a>
        		</c:if>
        	</c:forEach>
        	<c:if test="${param.p!=PageDate.pagetotal}">
        		<a class="next" href="searchServlet?${type }=${s }&p=${param.p+1 }&order=${order }#s">下一页</a>
        	</c:if>
