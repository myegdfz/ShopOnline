	...
	<div class="flexcontainer">
		...
		<div class="nav-wrap">
					...
					<!-- 商品分类循环go -->
						<c:forEach items="${floor}" var="bigType">
							<div class="item">
								<div class="product">
									<h4>
										<a href="searchServlet?bid=${bigType.id }&p=1">${bigType.name }</a>
									</h4>
								</div>
			...
			<c:forEach items="${floor }" var="bigType" varStatus="big">
				<div class="list">
					<h3>
						<span class="floor">${big.index+1 }F</span><span>${bigType.name}</span>
					</h3>
					<div class="category">
						<div class="icon">
							<ul class="tag clearfix" data-tpc="3">
								<c:forEach items="${bigType.smallTypeList }" var="sm">
									<a target="_blank" title="${sm.name }"
										href="searchServlet?sid=${sm.id }&p=1"> ${sm.name }</a>
								</c:forEach>
							</ul>
							<a href="searchServlet?bid=${bigType.id }&p=1" target="_blank">
