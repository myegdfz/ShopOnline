...
<div class="center_right" id="MemberCenterCtrl" style="display: block;">
	...
	<form method="post" id="fro" class="person-info" name="form" action="userDataUpdate.user">		
		<ul style="margin-left:40px;">
			...
			<li><span>性别：</span>
			<select name="sex">
				<c:if test="${userBean.sex!='男' }"><option value="男">男</option></c:if>
				<c:if test="${userBean.sex=='男' }"><option value="男" selected>男</option></c:if>
				<c:if test="${userBean.sex!='女' }"><option value="女">女</option></c:if>
				<c:if test="${userBean.sex=='女' }"><option value="女" selected>女</option></c:if>
				<!--  <option vlaue="男"  <c:if test="${userBean.sex == '男'}">selected="selected"</c:if> >男</option>
				<option vlaue="女"  <c:if test="${userBean.sex == '女'}">selected="selected"</c:if> >女</option>
				-->
			</select>
			</li>
